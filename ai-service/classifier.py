...
def _fetch_url_info(url: str) -> dict:
    """Obtiene título y descripción de una página web. Devuelve {} si falla."""
    try:
        headers = {"User-Agent": "Mozilla/5.0 (compatible; DigitalBrain/1.0)"}
        with httpx.Client(timeout=5, follow_redirects=True) as client:
            resp = client.get(url, headers=headers)
        html = resp.text[:8000]
        title_m = re.search(r'<title[^>]*>([^<]+)</title>', html, re.IGNORECASE)
        desc_m  = re.search(r'<meta[^>]+name=["\']description["\'][^>]+content=["\'](.*?)["\']', html, re.IGNORECASE | re.DOTALL)
        if not desc_m:
            desc_m = re.search(r'<meta[^>]+content=["\'](.*?)["\'][^>]+name=["\']description["\']', html, re.IGNORECASE | re.DOTALL)
...
def _find_short_list(note_text: str, first_idea: str) -> list | None:
    """
    Caso específico: el LLM ya identificó la primera idea (p.ej. 'pan').
    Busca esa palabra en la nota y extrae todos los ítems cortos que la siguen,
    separados por comas o 'y'. Sólo activa si TODOS los ítems son 1-3 palabras.
    """
    normalized = _Y_NORM.sub(', ', note_text.strip().rstrip('.!?'))
    idx = normalized.lower().rfind(first_idea.lower())
    if idx == -1:
        return None
    tail = normalized[idx:]
...
def _maybe_expand_enumeration(results: list[ClassificationResult], note_text: str) -> list[ClassificationResult]:
    ...
    # Caso B: la nota tiene lista pero el LLM sólo puso la primera idea (≥3 ítems)
    note_items = _find_list_in_note(note_text)
    if note_items and len(note_items) >= 3:
        return _expand_result(r, note_items)

    # Caso C: la nota tiene «X y Y» (2 ítems) y el LLM sólo devolvió X
    if r.idea and len(r.idea.split()) <= 4:
        short_items = _find_short_list(note_text, r.idea)
        if short_items and len(short_items) >= 2:
            return _expand_result(r, short_items)
...
def classify_note(note_text: str, existing_groups: list[dict], lang: str = "es") -> list[ClassificationResult]:
    ...
    # ── URL detection: enriquece el contexto con el contenido de la página ───
    url_match    = _URL_RE.search(note_text)
    detected_url = url_match.group(0).rstrip('.,)>') if url_match else None
    url_hint     = ""
    if detected_url:
        info = _fetch_url_info(detected_url)
        if info["title"] or info["description"]:
            parts = []
            if info["title"]:       parts.append(f'Título: "{info["title"]}"')
            if info["description"]: parts.append(f'Descripción: "{info["description"][:200]}"')
            url_hint = f'\n[Información del enlace — {", ".join(parts)}]'
    note_for_llm = note_text + url_hint

    def _annotate(results: list) -> list:
        """Propaga la URL detectada a cada resultado para que el backend la almacene."""
        if detected_url:
            for r in results:
                r.url = detected_url
        return results
    ...
    try:
        ...
        if tool_calls:
            results = _results_from_tool_calls(tool_calls, note_for_llm, existing_groups)
            if results:
                return _annotate(_maybe_expand_enumeration(results, note_for_llm))
    except Exception:
        pass  # fallback al método JSON

    # ── 2. Fallback: prompt JSON clásico ──────────────────────────────────────
    prompt = _build_classification_prompt(note_for_llm, existing_groups)
    raw_response = _call_ollama(prompt=prompt, system=SYSTEM_PROMPT, temperature=0.1)
    data = extract_json(raw_response)

    if isinstance(data, list):
        results = [_build_single_result(item, note_for_llm, existing_groups) for item in data if isinstance(item, dict)]
        for r in results[1:]:
            r.is_new_group        = False
            r.is_new_subgroup     = False
            r.inherit_parent_ideas = False
        ...
        return _annotate(_maybe_expand_enumeration(final, note_for_llm))

    return _annotate(_maybe_expand_enumeration([_build_single_result(data, note_for_llm, existing_groups)], note_for_llm))
