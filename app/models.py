...
class InboxEntry(Base):
    ...
    source_url = Column(String, nullable=True)        # URL original si la nota era un enlace
...
class Reminder(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
    # Burbuja vinculada: al disparar o eliminar el recordatorio se borra también
    entry_id   = Column(Integer, ForeignKey("inbox_entries.id"), nullable=True)